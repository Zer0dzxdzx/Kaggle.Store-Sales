...
| 判断问题 | 你需要给出的答案 |
| --- | --- |
| 哪个 EDA 发现最可能解释 Kaggle public score 偏高？ | 待回答 |
| 高零销量 family 应该单独建模，还是先加特征？ | 待回答 |
| 促销特征应该继续增强吗？如果增强，先做什么？ | 待回答 |
| fold 3 变差更可能来自时间漂移、促销分布变化，还是节假日/事件？ | 待回答 |
| 下一步应该先做误差分析，还是直接尝试 extended lag？ | 待回答 |

## 阶段 3 结论

阶段 3 的初步结论是：当前最值得优先进入阶段 4 误差分析的方向不是盲目换模型，而是分组看错误来源。
