...
| 判断问题 | 你需要给出的答案 |
| --- | --- |
| 哪个 EDA 发现最可能解释 Kaggle public score 偏高？ | fold 越靠近测试期 RMSLE 越高最可疑，说明模型在临近 public period 的泛化变差。但这只是线索，需要通过 fold 3 分组误差分析确认。 |
| 高零销量 family 应该单独建模，还是先加特征？ | 先不单独建模，先做 family 级误差分析。如果高零销量 family 确实贡献主要误差，再优先尝试零销量率、历史均值、低需求标记等特征。 |
| 促销特征应该继续增强吗？如果增强，先做什么？ | 应该考虑增强，但先按 `onpromotion` 分箱和 `family + promotion` 分组看误差。如果高促销样本误差更大，再加促销分箱或 family 级促销响应特征。 |
| fold 3 变差更可能来自时间漂移、促销分布变化，还是节假日/事件？ | 当前优先怀疑时间漂移，但也可能和促销、节假日、family/store 分布变化有关，需要用 fold 对比验证。 |
| 下一步应该先做误差分析，还是直接尝试 extended lag？ | 先做误差分析，不直接尝试 `extended` lag。先定位错误来源，再决定是否加 extended lag 或其他特征。 |

## 你的阶段 3 判断

当前你自己的正式判断是：

- public score 偏高最可疑的线索是 fold 越靠近测试期 RMSLE 越高，但这只是观察和假设，需要通过 fold 3 分组误差分析确认。
- 高零销量 family 暂时不单独建模，先做 family 级误差分析；如果高零销量 family 确实贡献主要误差，再加零销量率、历史均值、低需求标记等特征。
- 促销特征值得进一步检查，但第一步是按 `onpromotion` 分箱和 `family + promotion` 分组看误差，再决定是否增强促销特征。
- fold 3 变差当前优先怀疑时间漂移，但也可能来自促销、节假日、family/store 分布变化，需要后续验证。
...
| 层级 | 当前内容 |
| --- | --- |
| 观察 | fold 越靠近测试期 RMSLE 越高；零销量比例高；促销和销量相关；测试期促销更强。 |
| 假设 | public score 偏高可能来自临近测试期的时间漂移、促销分布变化或某些 family/store 的系统性误差。 |
| 验证 | 阶段 4 需要按 `family`、`store_nbr/cluster`、`onpromotion` 分箱和 fold 对比做误差分析。 |
| 结论 | 目前还不能断言具体原因，只能确定下一步应先做误差分析。 |
