...
- 目标：让 README 和文档导航更像正式作品集入口，让外部读者更快抓住项目价值。
- README 新增 5 分钟阅读路线、项目流程图和项目亮点表。
- README 补充数据公开说明，强调 Kaggle 原始数据不随仓库分发，`artifacts/` 和 `private_notes/` 不进入公开仓库。
- `docs/index.md` 改成按读者意图导航，包括复现、验证、防泄漏、实验记录、case study 和升级路线。

结果：

- 外部读者可以从 README 第一屏快速理解项目任务、结果、验证方式和证据入口。
- 文档导航从“文档列表”升级为“读者路线图”。

风险 / 问题：

- 这次只优化展示层，不改变模型、验证协议或报告结果。
- 如果后续重写 Git 历史或新增 LICENSE，需要继续同步 README 的公开说明。

下一步：

1. 补 LICENSE。
2. 补小样本端到端 CLI smoke test。
3. 如准备公开推送，先决定是否清理包含旧私有材料的 Git 历史。

#### 公开化清理：移出私有面试材料

- 目标：让公开 GitHub 仓库更像正式作品集，而不是面试备考资料夹。
- 将原公开面试讲稿和深挖文档移到本地私有目录。
- 在 `.gitignore` 中加入 `private_notes/`，避免私有面试材料进入公开仓库。
- 清理 README 和 `docs/index.md` 中指向私有面试材料的公开入口。

结果：

- 公开仓库保留项目说明、结果总结、case study、复现、验证协议、实验日志和报告。
- 面试话术和深挖材料仍保留在本地，但不会被 Git 跟踪。

风险 / 问题：

- `docs/project_log.md` 和 `docs/project_progress_table.md` 仍保留历史阶段记录，会提到曾经整理过面试材料，但不再公开具体话术文档。
- 如果未来 push 到远程，已在历史 commit 中出现过的文件仍可能存在于 Git 历史；如需彻底从历史中清除，需要单独做历史重写，本次不做。

下一步：

1. push 前确认公开入口没有死链接。
2. 如特别在意历史可见性，再讨论是否需要重写 Git 历史。
...
- 第 1 到第 7 天的作品集化闭环完成：README、最终结果、复现、测试、case study、CI 和文档导航。
...
- 目标：把 case study 压缩成本地私有沟通提纲，避免正式沟通时讲成流水账。
- 新增一份私有沟通提纲，后续已移动到本地私有目录，不再作为公开仓库文档展示。
- 文档内容包括：
  - 不同长度的项目摘要
  - 常见问题回答模板
  - 不同岗位方向版本
  - 不适合公开夸大的说法
  - 正式沟通前练习清单
- README 曾短暂加入私有材料入口，后续公开化清理已移除。
- `docs/resume_project_summary.md` 的下一步移除“补 interview talk-track”，改为 CI、稳定性优化和 live walkthrough checklist。

结果：

- 项目现在有独立的私有沟通提纲。
- case study 负责公开完整叙事，私有提纲负责口语化表达和问题准备。

风险 / 问题：

- 私有提纲仍需要真人反复练习，不能机械背诵。
- 文档还没有根据具体 JD 定制版本，后续可以按数据分析、数据科学、机器学习实习分别微调。

下一步：

1. 可选补 GitHub Actions CI，让 pytest 在 push / PR 时自动运行。
2. 如果准备具体岗位沟通，再在本地私有材料中根据岗位 JD 精简一版定制提纲。

### 2026-05-09

#### 作品集化第 5 天：case study 初版

- 目标：把项目从分散的日志、报告和实验记录，整理成一条作品集可展示的完整项目故事。
...
- `docs/resume_project_summary.md` 的下一步从“补测试/复现/case study”更新为 CI 和稳定性优化。
...
- case study 是第一版，后续还可以继续压缩成更短的对外摘要。
- 当前仍没有 CI，测试需要本地手动运行。

下一步：

1. 在本地私有材料中整理不同长度的项目摘要和问题回答模板。
...
2. 后续再补完整 case study，并在本地私有材料中整理沟通提纲。
...
- 文档补充了每张表的阅读方法、当前结果解读、不能下的结论和对外展示版本。
