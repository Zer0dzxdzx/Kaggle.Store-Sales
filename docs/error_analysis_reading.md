...
阶段 4 的核心结论是：优化前必须先定位误差来源，不能直接堆模型或盲目加特征。

本阶段按 family、store、promotion bin 和 fold 统计 RMSLE，发现误差集中在部分低销量或高零销量品类，且越靠近测试期的验证窗口表现越差。promotion bin 分析也修正了一个初始假设：当前最高 RMSLE 不在高促销样本，而在无促销样本。因此后续实验应优先围绕低需求品类、零销量行为和时间稳定性展开，而不是直接增强高促销特征。
